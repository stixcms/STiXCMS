...
<SCRIPT Language=Javascript>
	...
	function doAttach(id,nm) {
<?//	if($insertit) {	?>
		/*parent.opener.doInsertImageAction(id,nm);
		opener.document.art.att1.value=id;
		parent.opener.focus();
		parent.close();*/
<?	//} else {	?>
...
<?	//}	?>
	...
	parent.opener.document.artf2.<?=$obj?>.value=id;
	parent.opener.document.artf2.dummy<?=$obj?>.value=nm;
		//.att1.value=id;
		parent.opener.focus();
		parent.close();		
	}
...
<?	if($insertit) {	?>
	<td><a href=# onClick="doAttach('<?=$al->list[$i]->URI?>','<?=$al->list[$i]->title?>');">[insert]</a></td>
<?	} else {	?>
	<td><a href=# onClick="doAttach(<?=$al->list[$i]->att_id?>,'<?=$al->list[$i]->title?>');">[attach]</a></td>
<?	}	?>	
	</tr>
<?
	}
